...
class PaddingConvTranspose2d(nn.ConvTranspose2d):
    def __init__(
        ...
        if padding_mode == "circular":
            ...
            if stride == 1:
                ...
                )  # first sub kernel size : center and twice 1/s pixels on each side
                k2 = math.ceil(
                    (kernel_size - k1) / (stride - 1)
                )  # max of other sub kernel size
    ...
    def forward(self, input):
        if self.pad4transpose is not None:
            input = nn.functional.pad(input, self.pad4transpose, mode="circular")
        output = super(PaddingConvTranspose2d, self).forward(input)
        if self.pad4transpose is not None:
            output = output[
                :, :, self.crop_left : self.crop_right, self.crop_left : self.crop_right
            ]
